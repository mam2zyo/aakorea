...
핵심 서비스는 `user`, `general`, `store`, `heart` 네 개의 앱 도메인으로 구성됩니다.

- `user`: 공통 인증, 계정 상태, 최소 프로필, 배송지 마스터
- `general`: A.A. Korea 대표 앱
  - `public` = `basic`: 비로그인 사용자 대상 공개 정보, 모임 찾기, 안내 콘텐츠
  - `admin` = `service`: 관리자 및 봉사자용 조직/그룹/모임/공지/공개 콘텐츠 운영
- `store`
  - `public`: 도서 카탈로그, 주문 진입, 사용자 주문 조회
  - `admin`: 상품, 재고, 주문, 배송 운영
- `heart`
  - `public`: 월간지 소개, 구독 진입, 사용자 구독 조회
  - `admin`: 구독, 발송 이력, 만료 관리

`user`는 공통 로그인과 최소 공통 계정 정보만 담당하고, 실제 운영 데이터의 소유권은 `general`, `store`, `heart`가 각각 가진다. `general` 내부에서는 `public/basic`이 공개 조회 표면을, `admin/service`가 운영 표면을 담당한다.

## 현재 구현 상태

현재 저장소에는 아래 범위의 초기 구현이 포함되어 있습니다.

- 백엔드: `general.public(basic)` 공개 모임 조회 API, 그룹 상세 조회 API, `general.admin(service)` 지역연합/그룹/GSR 관리 API
- 프론트엔드: `general.public(basic)` 모임 검색 화면, 그룹 상세 화면, `general.admin(service)` 지역연합/그룹 관리 화면
