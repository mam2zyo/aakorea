...
- `service`: 관리자 및 봉사자용 조직/그룹/모임/공지/공개 콘텐츠 운영
- `member`: 공통 인증, 계정 상태, 최소 프로필, 배송지 마스터
- `store`: 도서 주문, 재고, 배송 처리
- `heart`: 월간지 구독, 발송 이력, 만료 관리

`member`는 공통 로그인과 최소 공통 계정 정보만 담당하고, 실제 권한과 운영 데이터의 소유권은 `service`, `store`, `heart`가 각각 가진다.
