...
> "고통받는 알코올 중독자가 AA 모임에 관한 정보를 빠르게 찾을 수 있고, 기존 멤버들과 관련 전문가들에게 유용한 자료를 제공하며, 관리자 및 봉사자들에게 효율적인 업무 환경을 제공해주는 것을 목표로 합니다."

본 프로젝트는 기존 레거시 시스템(www.aakorea.org) 의 파편화된 정보 구조를 개선하고, 최신 웹 기술 스택을 활용하여 **사용자 중심(User-Centric)의 직관적인 웹앱**을 재구축하는 프로젝트입니다.
...
- `basic`: 기존 웹사이트 내용 승계, 로그인이 불필요한 서비스, 모임 찾기 기능 강화를 비롯한 UI/UX 고도화
- `service`: 관리자 페이지 접근 경로, 지역연합 및 그룹 관리, 총회 구조에 대응하는 위원회 게시판 기능, PWA 앱 전환 1순위
