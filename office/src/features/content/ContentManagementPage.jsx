...
export function ContentManagementPage({ onError, onNavigate, onSuccess }) {
  ...
  async function loadContentPageWorkspace() {
    ...
    try {
      const data = await contentApi.getContentPages()
  ...
  async function handleSave(event) {
    ...
    try {
      if (isEditMode) {
        await contentApi.updateContentPage(uploadId, {
          key: uploadKey,
          title: uploadTitle,
        ...
        await contentApi.uploadContentPage({
          key: uploadKey,
          title: uploadTitle,
          published: uploadPublished,
          file: uploadFile,
  ...
  async function handleDelete(contentPage) {
    ...
    try {
      await contentApi.deleteContentPage(contentPage.id)
      await loadContentPageWorkspace()
  ...
  async function handlePublish(contentPage, published) {
    ...
    try {
      await contentApi.publishContentPage(contentPage.id, { published })
      await loadContentPageWorkspace()
    ...
    <div className="office-flat-page">
      ...
      <div className="office-flat-page__workspace">
        {contentPages.length === 0 ? (
          ...
          <div className="office-table office-table--content" role="table" aria-label="안내 페이지 목록">
            <div className="office-table__header" role="row">
              <span className="office-table__heading" role="columnheader">번호</span>
              <span className="office-table__heading" role="columnheader">페이지 key</span>
              <span className="office-table__heading" role="columnheader">제목</span>
              <span className="office-table__heading" role="columnheader">상태</span>
            ...
            {filteredContentPages.map((contentPage, index) => (
              ...
              >
                ...
                <span className="office-table__cell" data-label="페이지 key">
                  {contentPage.key}
                </span>
                <span
                  className="office-table__cell office-table__cell--primary"
                  data-label="제목"
                  style={{ whiteSpace: 'nowrap', textOverflow: 'ellipsis', overflow: 'hidden' }}
                >
                  <strong>{contentPage.title}</strong>
                </span>
                <span className="office-table__cell" data-label="상태">
                  <span
                ...
                <span className="office-table__cell office-table__cell--action" data-label="관리">
                  <div className="office-table__action-cluster">
                    <button
                      className="ghost-button ghost-button--small"
                      type="button"
      ...
      {uploadOpen && (
        <div className="office-overlay" role="presentation">
          <div
              ...
              <div className="office-group-edit-sheet__status-toggle">
                ...
                <button
                  ...
                  className={`office-group-edit-sheet__switch${
                    uploadPublished ? ' office-group-edit-sheet__switch--active' : ''
                  }`}
                  role="switch"
                  type="button"
