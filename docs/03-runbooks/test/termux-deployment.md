...
## 3. 콘텐츠 관리 가이드 (HTML vs JSX)

페이지의 성격에 따라 관리 방식을 선택한다.

### 3.1 일반 안내 페이지 (HTML 방식)
- **용도**: 텍스트와 이미지 위주의 단순 안내 페이지.
- **방법**: 어드민 UI에서 HTML 업로드.
- **저장 위치**: `~/aakorea/contents/`
- **장점**: 개발자 없이 관리자가 실시간으로 수정 가능.

### 3.2 인터랙티브 페이지 (JSX 방식)
- **용도**: 카카오맵, 복잡한 폼, 실시간 데이터 연동이 필요한 페이지.
- **방법**: 프론트엔드 소스 `src/public/features/content/components/pages/`에 JSX 컴포넌트 작성 후 `registry.jsx`에 등록.
- **장점**: 리액트 기능을 활용한 고도화된 기능 구현 가능.
