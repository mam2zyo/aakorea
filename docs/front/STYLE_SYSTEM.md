...
AAKorea Main의 스타일 시스템은 `@/shared/styles/`에 정의된 3계층 디자인 토큰 체계를 따릅니다. 이는 관리자(`admin/`)와 공개 사이트(`public/`) 모두에서 동일한 시각적 언어를 사용하도록 보장합니다.
...
- **파일**: `@/shared/styles/tokens.css` 내 `:root`
...
- **파일**: 각 도메인용 스타일 시트 (예: `@/admin/styles/`, `@/public/styles/`)
...
## 4. 구현 지침
1. **Literal 금지**: CSS 파일에서 `1.5rem` 등 하위 값을 직접 쓰지 말고 토큰을 사용하세요.
2. **별칭 사용**: 스타일 시트 접근 시 가급적 `@/shared/styles` 별칭을 사용하여 경로를 통일합니다.
3. **테마 전환**: `document.documentElement`의 `data-public-theme` 속성 값을 변경하여 전체 테마를 즉시 전환할 수 있습니다.
