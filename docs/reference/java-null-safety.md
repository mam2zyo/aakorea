...
## 1. 개요
현재 프로젝트에서는 스프링 프레임워크 5+ 및 6+ 버전을 사용하고 있으며, 이는 JSR-305 어노테이션을 통해 API 수준에서 널 안정성을 보장하려 합니다. VS 코드(Eclipse JDT 엔진)는 이를 매우 엄격하게 해석하여, 잠재적으로 널이 될 수 있는 객체(External Libraries, JDK 등에서 오는 값)를 `@NonNull`이 기대되는 곳에 사용할 때 경고를 발생시킵니다.
