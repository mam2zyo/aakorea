...
class AdminGsrControllerTest {
    ...
    void supportsBasicCrud() throws Exception {
        mockMvc.perform(post("/api/admin/service/gsrs")
        ...
        mockMvc.perform(put("/api/admin/service/gsrs/{gsrId}", gsrId)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        mockMvc.perform(get("/api/admin/service/gsrs"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.gsrs", hasSize(1)));

        mockMvc.perform(delete("/api/admin/service/gsrs/{gsrId}", gsrId))
