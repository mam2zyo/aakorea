...
class AdminNoticeControllerTest {
    ...
    void managesNoticeCrudAndPublicExposure() throws Exception {
        ...
        AdminNoticeRequest createRequest = new AdminNoticeRequest(
                "이번 주 장소 변경",
                "서울역 인근 회의실로 이동합니다.",
                io.step5.aakorea.modules.service.notice.domain.NoticeType.TEMP_CHANGE,
                true,
                LocalDateTime.now().minusHours(2),
                LocalDateTime.now().plusDays(1)
        );

        mockMvc.perform(post("/api/admin/groups/{groupId}/notices", group.getId())
        ...
        mockMvc.perform(put("/api/admin/groups/{groupId}/notices/{noticeId}", group.getId(), noticeId)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        mockMvc.perform(delete("/api/admin/groups/{groupId}/notices/{noticeId}", group.getId(), noticeId))
                .andExpect(status().isNoContent());

        mockMvc.perform(get("/api/admin/groups/{groupId}/notices", group.getId()))
                .andExpect(status().isOk())
    ...
    void rejectsInvalidDisplayWindow() throws Exception {
        ...
        AdminNoticeRequest request = new AdminNoticeRequest(
                "잘못된 공지",
                "종료 시각이 더 빠른 경우",
                io.step5.aakorea.modules.service.notice.domain.NoticeType.GENERAL,
                true,
