import { BASIC_DAY_OF_WEEKS, BASIC_PROVINCES } from "./basic-reference";

const provinceMap = Object.fromEntries(
  BASIC_PROVINCES.map((item) => [item.value, item.label]),
);

const dayOfWeekMap = Object.fromEntries(
  BASIC_DAY_OF_WEEKS.map((item) => [item.value, item.label]),
);
